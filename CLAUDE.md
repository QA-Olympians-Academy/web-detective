...
- **Ch4 self-healer: use `WORKSHOP_MODEL=llama3.1:latest`.** The default `deepseek-r1:8b` is impractical for the healing loop — it spends its whole token budget inside `<think>…</think>` and returns empty output, so no candidate selectors are produced. `llama3.1` returns clean JSON selector arrays quickly. Run: `WORKSHOP_MODEL=llama3.1:latest npx tsx examples/ch4-self-healing/run-healer.ts`.
